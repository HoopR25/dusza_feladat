...
def menu():
    cls()
    cprint("Udvozollek a Danareen-ben!\n", "blue")
...
def dungeon():
    ...
    for i in range(len(game_kazamatak)):
        ...
        if game_kazamatak[i][1] == "egyszeru" or game_kazamatak[i][1] == "nagy":
...
def pakliallito(kiv_kaz):
    ...
    print("")
    cprint(f"Adja meg szokozokkel elvalaszta a kivalasztott kartyak nevet sorrendben, max. {round(len(cards.gyujtemeny) / 2)}".center(os.get_terminal_size().columns), "blue")
    while True:
        if keyboard.is_pressed("esc"):
            menu()
            break
        ...
        if jatekh["eletero"] <= 0:
            ...
            if jatekh["tipus"] != kazh["tipus"]:
                    ...
                    if fight.tipusok[ktipus] == fight.tipusok[jtipus - 1] or fight.tipusok[ktipus] == fight.tipusok[jtipus + 1]:
                        ...
                        jatekh["eletero"] -= int(jatekh["sebzes"] / 2)
